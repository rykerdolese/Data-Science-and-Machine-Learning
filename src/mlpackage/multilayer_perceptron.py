...
class Layer:
    """
    Single neural network layer with activation function.
    ...
    actFun_type : str, default='tanh'
        Activation function type: 'tanh', 'sigmoid', or 'relu'.
    seed : int, default=0
    ...
    ----------
    W : np.ndarray
        Layer weights of shape (input_dim, output_dim).
    b : np.ndarray
        Layer biases of shape (1, output_dim).
    a : np.ndarray
...
class MLP:
    ...
    actFun_type : str, default='tanh'
        Activation function for hidden layers: 'tanh', 'sigmoid', or 'relu'.
    ...
    def __init__(self, layer_dims, actFun_type='tanh', reg_lambda=0.01, seed=0):
        ...
        for i in range(len(layer_dims) - 2):
            self.layers.append(Layer(layer_dims[i], layer_dims[i + 1], actFun_type, seed + i))
    ...
    def calculate_loss(self, X, y):
        ...
        reg_loss = 0.5 * self.reg_lambda * (np.sum(self.W_out**2) + sum(np.sum(layer.W**2) for layer in self.layers))
    ...
    def backprop(self, X, y):
        ...
        dW_out = np.dot(self.layers[-1].a.T if self.layers else X.T, delta_out) / num_samples
        ...
        for i in reversed(range(len(self.layers))):
            ...
            deltas = np.dot(deltas, self.W_out.T if i == len(self.layers) - 1 else self.layers[i + 1].W.T) * layer.diff_actFun(layer.z)
